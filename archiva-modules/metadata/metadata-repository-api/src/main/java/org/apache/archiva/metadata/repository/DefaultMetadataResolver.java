...
argument_list|)
decl_stmt|;
comment|/**      *<p>      * FIXME: this needs to be configurable based on storage type - and could also be instantiated per repo. Change to a      * factory, and perhaps retrieve from the session. We should avoid creating one per request, however.      *</p>      *<p>      * TODO: Also need to accommodate availability of proxy module      * ... could be a different type since we need methods to modify the storage metadata, which would also allow more      * appropriate methods to pass in the already determined repository configuration, for example, instead of the ID      *</p>      */
annotation|@
name|Inject
annotation|@
name|Named
...
argument_list|)
expr_stmt|;
comment|// just invalidate cache entry
name|String
name|cacheKey
init|=
name|repoId
