...
condition|)
block|{
comment|// check if the repository is already queued to be scanned
if|if
condition|(
name|scheduler
operator|.
...
condition|)
block|{
comment|// no need to execute the consumers since repo is already queued
return|return;
block|}
else|else
block|{
comment|// schedule the task
name|RepositoryTask
name|task
init|=
operator|new
