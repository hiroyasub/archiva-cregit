...
expr_stmt|;
block|}
comment|//Lets get the versions for the artifact we just found and display them
comment|//Yes, this is in the lucene index but its more challenging to get them out when we are searching by project
comment|// TODO: do we still need to do this? all hits are already filtered in the NexusRepositorySearch
comment|//      before being returned as search results
for|for
control|(
name|SearchResultHit
name|resultHit
